...
class KimCNNTwoStreamsFTLate(nn.Module):
    def __init__(self, word_model, word_model2, **config):
        ...
        embedding_dim2 = config.get("hidden_size2", 1024)
        ...
        n_c = config.get("channels", 13)#13 #word_model.n_channels        
        n_c2 = config.get("channels2", 25)#13 #word_model.n_channels
        ...
        self.conv_layers2 = [nn.Conv2d(n_c2, n_fmaps, (w, embedding_dim2), padding=(w - 1, 0)) for w in weight_lengths]
        for i, conv in enumerate(self.conv_layers2):
            self.add_module("convv{}".format(i), conv) # since conv_layers is a list, we need to add the modules manually
        ...
        self.fc = nn.Linear(2*len(self.conv_layers) * n_fmaps, config.get("n_labels", 2)) 
    ...
    def forward(self, x):
        ...
        x1 = [nn_func.relu(conv(x1)).squeeze(3) for conv in self.conv_layers] # squeeze(3) to get rid of the extraneous dimension
        x2 = [nn_func.relu(conv(x2)).squeeze(3) for conv in self.conv_layers2] # squeeze(3) to get rid of the extraneous dimension
        # max-pool over time as mentioned in the paper
        x1 = [nn_func.max_pool1d(c, c.size(2)).squeeze(2) for c in x1]
        # Concatenate along the second dimension:
        x1 = torch.cat(x1, 1)

        x2 = [nn_func.max_pool1d(c, c.size(2)).squeeze(2) for c in x2]
        # Concatenate along the second dimension:
        x2 = torch.cat(x2, 1)
        x = torch.cat([x1, x2], dim = 1)
        # Apply dropout
