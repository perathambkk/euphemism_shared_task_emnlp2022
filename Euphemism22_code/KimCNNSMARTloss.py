...
class KimCNN(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.word_model(**x, output_hidden_states=True).hidden_states # shape: (batch, channel, sent length, embed dim)
        ...
        state = eval(x) # logitd
        if self.training:
            loss = self.criterion(nn_func.softmax(state, dim=1), labels)
            loss += self.weight * smart_loss_fn(x, state)
        else:
            loss = None
        return loss, state